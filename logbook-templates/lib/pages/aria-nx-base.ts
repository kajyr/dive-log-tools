 ...
 */
function page(doc: Doc, dive: Partial<Dive>) {
  doc.addPage({
    size: 'A5',
  });
  ...
  const [nextX, nextY] = header(doc, 'SCHEDA ARIA - NITROX BASE', MARGINS.left, MARGINS.top, contentWidth, 30);
  ...
  panel(doc, nextX, nextY, contentWidth, panelH, 5, (doc, startX, startY, width, height) => {
...

    {
      ...
      first((doc, startX, startY, width, height) => {
        ...
        const { r, rowH } = rows(startY + headerLineHeight + 5, height - headerLineHeight, 4, 5);
      ...
      second((doc, startX, startY, width, height) => {
        ...
        const { r, rowH } = rows(startY + headerLineHeight + 5, height - headerLineHeight, 3, 5);

        doc.fontSize(FONT_SIZE_FIELDS);

        const [muta, zavorra] = columns(doc, [55, 45], startX, r[0], width, rowH, 5);
        const [bombola, gasArea] = columns(doc, [40, 60], startX, r[1], width, rowH, 5);
...

    {
      ...
      const [first, second, third, fourth] = columns(doc, [25, 36, 17, 21], startX, nextY, width, groupHeight, 10);
      ...
      first((doc, x, y, w, h) => {
        ...
        fieldWithUpperLabel(doc, x + 15, r[0], 50, rowH, 'Int. di sup.', dive.surfaceInterval);
      ...
      second((doc, x, y, w, h) => {
        ...
        fieldWithUpperLabel(doc, x + 5 + baseFW + 15, r[3], baseFW, rowH, 'T tabella', null, {
          bold: true,
          sublabel: '(min)',
        });
      });
      ...
      fourth((doc, x, y, w, h) => {
        ...
        field(doc, x, centerY(r[2], fH, rowH), w - 15, fH, 'Bar', gas.pressureEnd ? Math.round(gas.pressureEnd) : '', {
          ...fieldOptions,
          sublabel: 'fine',
        });
    ...
    // Annotazioni
    const ann = block(doc, startX, nextY, width, height - nextY + startY);
