...
const component: Component = (doc, area, dive) =>
  panel(doc, area, 3, (a) => {
    ...
    title(doc, 'CONDIZIONI', tMain.x, tMain.y, 9, { width: a.w });
    title(doc, 'AMBIENTALI', tAmb.x, tAmb.y, 6, { align: 'center', height: area.h, valign: 'center', width: area.w });
    title(doc, 'PERSONALI', tPers.x, tPers.y, 6, { align: 'center', height: area.h, valign: 'center', width: area.w });
    ...
    const [labels, inputs] = columnsFixed(doc, [25, null], personali, 2);
    box(labels, ({ x, w }) => {
